...
const critic: GraphCriticState = { id: 'human-check', title: '문서 검토', target: 'spec', deps: ['why'], kind: 'human', requestId: 'request-one', status: 'WAITING_HUMAN', claimedBy: null, blockedReason: null };
...
test('Human icon stays requested until claim and then shows a running review', () => {
  ...
  assert.match(criticPresentation(critic).accessibleLabel, /문서 검토.*Human.*담당자 기다림/);
...
test('omitted Critic is never actionable or shown as queued, even alongside a successful review', () => {
  ...
  assert.match(state.label, /포함되지 않음/);
...
test('upstream failure does not misrepresent the blocked Critic as a failed evaluation', () => {
  ...
  assert.equal(state.tone, 'requested'); assert.equal(state.mark, 'blocked'); assert.match(state.label, /진행 불가/);
