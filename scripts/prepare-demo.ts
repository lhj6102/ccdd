...
export async function prepareDemo({root=join(process.env.CCDD_DEMO_HOME || join(homedir(),'.local','share','ccdd'),'demo-v5')}: {root?:string}={}):Promise<DemoManifest>{
  root=resolve(root);
  ...
  const profile:AgentProfile={kind:'agent',provider:'openai-codex',model:'gpt-5.6-sol',reasoning:'medium'};
