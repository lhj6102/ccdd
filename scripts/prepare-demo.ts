...
import type { RepoConfig, AgentProfile } from '../src/contracts.js';
...
export async function prepareDemo({root=join(process.env.CCDD_DEMO_HOME || join(homedir(),'.local','share','ccdd'),'demo-v8')}: {root?:string}={}):Promise<DemoManifest>{
  root=resolve(root);
  const manifestPath=resolve(root,'manifest.json');
  try {
    const existing=JSON.parse(await readFile(manifestPath,'utf8'));
    if(existing.version!==8 || existing.scenarios?.length!==4)throw new Error('Incompatible demo manifest.');
  ...
  const config:RepoConfig={artifacts:{why:{type:'markdown',path:'why.md',basis:true},spec:{type:'markdown',path:'spec.md'},tests:{type:'code',path:'tests'},implementation:{type:'code',path:'implementation'}},artifactTypes:{
  ...
  const manifest={version:8,repoId:'local',name:'한 번에 집중할 일',repoPath:scenarios[0].repoPath,scenarios};
