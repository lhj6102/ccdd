...
export function readChanges(db: DatabaseSync, runId: string, stateDir: string, { after = 0, limit = 100 }: ChangeOptions = {}) {
  ...
  const changes = rows.slice(0, limit).map(row => ({ cursor: Number(row.cursor), requestId: String(row.request_id), criticId: String(row.critic_id), status: String(row.status),
    ...
    result: row.result_ref ? { verdict: String(row.status), ...semanticResult(store.get<Record<string, unknown>>(String(row.result_ref))), reference: reviewReference(stateDir, String(db.prepare('SELECT run_id FROM requests WHERE id=?').get(row.request_id)!.run_id), String(row.request_id)) } : null,
