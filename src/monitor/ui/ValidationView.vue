...
const actionLabels = { BLOCKED: 'Blocked by dependency verdict', WAIT_DEPENDENCY: 'Waiting for dependency evidence', REUSE: 'Reuse verdict', COALESCE: 'Share active review', EXECUTE: 'Ready to review', WAIT: 'Execution not ready', ACTIVE: 'In progress', FAILED: 'Execution needs attention' };
</script>
...
<template>
  <section class="validation-view" aria-label="Current input validation status" :aria-busy="loading">
    ...
    <template v-if="result">
      ...
      <p class="validation-counts">Reuse {{ result.plan.counts.reuse }} · Coalesce {{ result.plan.counts.coalesce }} · Gated {{ result.plan.counts.gated }} · Ready to review {{ result.plan.counts.execute }} · Execution not ready {{ result.plan.counts.wait }}</p>
