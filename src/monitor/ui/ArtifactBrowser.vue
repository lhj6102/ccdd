...
defineProps<{ projectId: string; requestId: string; artifacts: ArtifactReferenceMetadata[]; preview?: 'tools' | 'historical'; humanReview?: boolean }>();
...
<template>
  <div class="artifact-browser">
    ...
    <p class="artifact-message">{{ preview === 'historical' ? 'Historical reviews are available for result lookup only.' : 'Inspect these folders with their Artifact-owned review tools.' }}</p>
