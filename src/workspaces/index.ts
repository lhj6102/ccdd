...
interface Inspection { hash: string; metadataHash: string; entries: WorkspaceEntry[] }
...
function observe(root: string, mode: WorkspaceMode, externalSignal?: AbortSignal, onProgress?: (progress: WorkspaceScanProgress) => void) {
  ...
  let scanning: Promise<Inspection> | undefined;
...
export async function prepareWorkspace({ repoPath, stateDir, mode = 'copy', signal }: { repoPath?: string; stateDir?: string; mode?: WorkspaceMode; signal?: AbortSignal } = {}): Promise<WorkspaceHandle> {
