...
export async function startServer({repoPath,stateDir=resolve(pkgRoot,'.ccdd/state'),manifest,port=4317,codexPath=process.env.CCDD_CODEX_PATH||bundledCodexPath,humanInbox=false,executors:injected}={}){
  await mkdir(stateDir,{recursive:true});
  const alarmMethods=humanInbox?[{id:'local-inbox',kind:'inbox',notify:async request=>{await appendFile(resolve(stateDir,'human-inbox.jsonl'),JSON.stringify({requestId:request.id,at:new Date().toISOString(),message:`리뷰 요청: ${request.title}`})+'\n');}}]:[];
  ...
  const server=http.createServer(async(req,res)=>{
    try{
      ...
      if(p==='/api/health'&&req.method==='GET')return send(res,200,{ok:true,version:'0.1.0',repoId:manifest?.repoId||'demo',humanAlarmMethods:humanInbox?['local-inbox']:[]});
  ...
  return {server,broker,url:`http://127.0.0.1:${actualPort}`,close:async()=>{await new Promise(ok=>server.close(ok));await broker.close();}};
}
