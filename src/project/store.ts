...
export function readEvidence(database: DatabaseSync): ValidationEvidence[] {
  const rows = database.prepare("SELECT data FROM requests WHERE status IN ('GREEN','RED') AND json_extract(data, '$.validationInput.version') = 2 ORDER BY json_extract(data, '$.completedAt'), rowid").all();
