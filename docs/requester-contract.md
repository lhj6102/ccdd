...
그룹 구성은 선행 평가 조건이 아닙니다. 그룹의 판정은 그룹을 직접 대상으로 삼은 Critic들만 집계하고 멤버의 판정과 서로 전파하지 않습니다. `deps`·`basis`와 실행 대기 규칙은 그룹에도 독립적으로 적용됩니다. 그룹과 이미지 도구는 현재 소스에서 지원하며 기존 v1.0.0 배포 파일에는 포함되지 않습니다.
