...
See [getting started](getting-started.md) for setup and [v5.1.0 release notes](releases/v5.1.0.md) for the current changes.

## Current release: 5.1

Plans now quote active request adoption as `COALESCE` with `counts.coalesce`
and an optional submission lease deadline. These additive API changes require
consumers to handle the new action and counter. Existing 5.0 state and reuse
identities remain valid; no new state directory or key transition is required.
...
CCDD 5.1.0 supports Node.js 22 LTS (22.19.0 or later). Install matching versions of the three packages:

```sh
npm install --ignore-scripts @ccdd/core@5.1.0 @ccdd/project@5.1.0 @ccdd/default-tools@5.1.0
npx ccdd-project config check
npx ccdd-project tools check
```

`@ccdd/core` supplies definitions. `@ccdd/project` supplies validation, the CLI, Broker, Executors, and monitor. `@ccdd/default-tools` is optional when all tools are custom. Project and default-tools 5.1.0 target core `>=5.0.0 <6`: the core SDK is unchanged and the new planning behavior does not require new core APIs.
...
```sh
gh workflow run release.yml --ref main -f tag=v5.1.0
```
