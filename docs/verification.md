...
The original v0.1.0-demo Release, private at the time of this verification, included an MP4, npm package, actual results in `verification.json`, and restart comparison in `restart-verification.json`. Its downloadable assets were retired when distribution moved to npm. The [source tag](https://github.com/lhj6102/ccdd/tree/v0.1.0-demo) and this historical verification account remain. The original JSON included only final evaluations and tool-call evidence, without authentication data or private reasoning.
