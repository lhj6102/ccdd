...
`ccdd-project status` and `plan` capture a consistent current input, evaluate its config, hash declared Artifact paths, and read actual evidence in a readonly SQLite transaction. They do not create a database, tickets, notifications or review results, and execute no review tools or Providers. DAG recursion memoizes only within a query. No Artifact stale flag or push invalidation is persisted. The captured workspace hash identifies a coherent observation; it is not the reuse key for every Critic.
