...
Submission hashing may prepare immutable nodes in yielding batches before the atomic Run publication. These batches contain no tickets, events, mutable lifecycle state or provisional evidence. A canceled submission can leave unreferenced immutable nodes, but no partial Run. The publication transaction revalidates every prepared node's exact canonical bytes and makes current evidence/coalescing decisions; no writer lock spans an await. Input is revalidated after asynchronous preparation. Submission object/hash memoization is scoped to the privately owned input graph and never reused for mutable caller objects across submissions.
