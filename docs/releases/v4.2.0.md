# CCDD 4.2.0 — Actionable schema errors and final-result repair

This release makes tool argument errors actionable and gives invalid final Agent
responses one bounded format-only repair turn. All three packages move together
to 4.2.0; Node.js support remains 22 LTS, at least 22.19.0.
...
- ValidationInput remains version 2 with no database migration. Package version
  4.2.0 participates in executor identity, so reviews do not silently reuse results
  from the previous executor behavior.
...
Unicode and persisted-envelope bounds, bounded schema diagnostics, redaction,
...
Argument values, raw TypeBox messages and arbitrary exception text are never
forwarded. The safe diagnostic boundary
