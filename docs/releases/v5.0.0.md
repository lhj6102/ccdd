# CCDD 5.0.0 — Contract, reuse and concurrency

This major release aligns all three packages at 5.0.0; Node.js support remains 22 LTS, at least 22.19.0. It changes requester results and reuse identity, improves Critic efficiency, and exposes execution and identity concurrency. Follow [migration](../migration-v5.md) before upgrading; see [contracts](../contracts.md) for the complete API behavior.
...
and two-process lease-expiry races. Invalid/future lease metadata fails closed;
monotonic deadlines prevent clock rollback from extending waits. Dead owners
are reconciled before source adoption, so the first new submission after worker
death executes its own ticket; existing followers fail rather than replaying
partial source work.
