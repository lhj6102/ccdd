...
One npm package; broker and executors are separate bounded contexts. Artifact Runner resolves request payload references into scoped viewer tools. Reviews use whole-workspace lock or immutable content-hash copies. No daemon or observer server is included; request-scoped workers own execution. Keep state and outputs outside reviewed input.
